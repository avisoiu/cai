...
class BaselineAgent(ArtificialBrain):
    def __init__(self, slowdown, condition, name, folder):
        ...
        self._cleared_doors_by_human = []
        self._found_victims = []
        self._collected_victims = []
        self._victims_punished_for = []
    ...
    def room_searched_by_human(self, room_name):
        return room_name in self._searched_by_human and room_name not in self._rooms_punished_for

    def victim_rescued_by_human(self, victim):
    ...
    def alter_belief(self, belief, modifier, state):
        ...
        with open('trust_logs.csv', mode='a+', newline='') as file:
            csv_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            csv_writer.writerow([self.get_trust(), self._gains, self._losses, self.get_competence(),
                                 self.get_willingness(), self.get_competence(), state['rescuebot']['score'],
    ...
    def decide_on_actions(self, state):
        ...
        if not self._loaded_beliefs:
            self._trustBeliefs = self._loadBelief(self._team_members, self._folder)
            self._loaded_beliefs = True
        self._trustBelief(self._team_members, self._folder, self._received_messages, state)
                ...
                for vic in remaining_vics:
                    ...
                    if vic in self._found_victims and vic not in self._todo:
                        ...
                        if 'critical' in vic or (
                                'mild' in vic and (not self.is_trusted() or self._distance_human == 'close')):
                            self._rescue = 'together'
                ...
                for info in state.values():
                    if 'class_inheritance' in info and 'ObstacleObject' in info['class_inheritance'] and 'rock' in info[
                        ...
                        if self.received_messages_content and self.received_messages_content[
                            ...
                            # If the human communicated with me to remove the obstacle,
                            # we increase the willingness according to the distance between them and the human
                    ...
                    if 'class_inheritance' in info and 'ObstacleObject' in info['class_inheritance'] and 'tree' in info[
                        ...
                        # Remain idle until the human communicates what to do with the identified obstacle
                            ...
                            if (self._waiting_ticks > self.seconds_to_ticks(time_to_wait) and not self._answered) or (
                                ...
                                self._send_message(
                                    'Removing tree blocking ' + str(self._door['room_name']) + ' as ' + reason,
                                    'RescueBot')
                    ...
                    if 'class_inheritance' in info and 'ObstacleObject' in info['class_inheritance'] and 'stone' in \
                        ...
                        if self._answered == False and not self._remove and not self._waiting:
                            if not self.is_trusted:
                                ...
                                self._send_message('Removing stones blocking ' + str(
                                    self._door['room_name']) + ' alone. I do not trust you.',
                                                   'RescueBot')
                                self._phase = Phase.ENTER_ROOM
                                self._remove = False
                                return RemoveObject.__name__, {'object_id': info['obj_id']}
                        ...
                        if self.received_messages_content and self.received_messages_content[
                            ...
                            # If the human communicated with the robot to remove the stone while the robot was not in their visual range,
                            # we increase their willingness, as the human will waste time by helping
                            if self._distance_human == 'far':
                                self.alter_belief("willingness", 0.1, state)
                            else:
                        ...
                        if self.received_messages_content and self.received_messages_content[
                            ...
                            # If the human decided to collaborate with robot on obstacle removal,
                            # the willingness is increased based on the distance between human and robot
                            if self._distance_human == 'far':
                                self.alter_belief("willingness", 0.05, state)
                            else:
                                self.alter_belief("willingness", 0.1, state)
                        ...
                        # Remain idle until the human communicates what to do with the identified obstacle
                            ...
                            if (self._waiting_ticks > self.seconds_to_ticks(time_to_wait) and not self._answered) or (
                                ...
                                reason = "you did not answer." if not self._answered else "you did not come."
                                self._send_message(
                                    'Removing stones blocking ' + str(self._door['room_name']) + ' as ' + reason,
                                    'RescueBot')
            ...
            if Phase.FOLLOW_ROOM_SEARCH_PATH == self._phase:
                    ...
                    for info in state.values():
                        if 'class_inheritance' in info and 'CollectableBlock' in info['class_inheritance']:
                            ...
                            if self.victim_rescued_by_human(vic):
                                # If the human lied about rescuing a victim, we decrease the competence by 0.5
                ...
                if self._goal_vic in self._found_victims and self._goal_vic not in self._room_vics and \
                    ...
                    self._send_message(self._goal_vic + ' not present in ' + str(self._door[
                                       ...
                                       'RescueBot')
                    # If the robot was told by the human to search in a room, and it found nothing there,
                    # the willigneness of the human is decreased, as they lied
                ...
                if self.received_messages_content and self.received_messages_content[
                    ...
                    # If the human communicated to the robot to rescue a victim, we increase their willingness for collaborating with the robot
                    # We also take into consideration the human-robot distance
                    if self._distance_human == 'far':
                        self.alter_belief("willingness", 0.05, state)
                    else:
                        self.alter_belief("willingness", 0.1, state)
                ...
                if self.received_messages_content and self.received_messages_content[
                    ...
                    self._send_message('Picking up ' + str(self._recent_vic) + ' in ' + self._door['room_name'] + '.',
                                       'RescueBot')
                    self._rescue = 'alone'
                    self._answered = True
                    self._waiting = False
                ...
                if self.received_messages_content and self._waiting and self.received_messages_content[
                    ...
                    if (self._waiting_ticks > self.seconds_to_ticks(time_to_wait) and not self._answered) or (
                        ...
                        if 'mild' in self._recent_vic:
                            reason = "you did not answer." if not self._answered else "you did not come."
                            self._send_message(
                                'Picking up ' + str(self._recent_vic) + ' in ' + self._door[
                                    'room_name'] + ' alone as ' + reason,
                                'RescueBot')
                            # If the human did not come/answer to save the mildly injured victim,
                            # we decrease their willingness and rescue it alone
                            self.alter_belief('willingness', -0.2, state)
                            self._rescue = 'alone'
                            self._answered = False
                            self._waiting = False
                            ...
                            # If the human did not come/answer to save the critically injured victim,
                            # we decrease their willingness and continue,
                            # as the human does not want to collaborate
                            self._answered = True
                            self._waiting = False
                            self._todo.append(self._recent_vic)
                            self._recent_vic = None
                            ...
                            self._phase = Phase.FIND_NEXT_GOAL
                            self._send_message(
                                'Continue without picking up' + str(self._recent_vic) + ' in ' + self._door[
                                    'room_name'] + ' as you did not answer.', 'RescueBot')
                    else:
                        return None, {}
                            ...
                            if not self._waiting:
                                ...
                                if self._waiting_ticks >= self.seconds_to_ticks(25):
                                    ...
                                    if len(objects) == 0 and 'critical' in self._goal_vic:
                                        self._phase = Phase.FIND_NEXT_GOAL
                                        self._waiting = False
                                        self._send_message("Stopped waiting on top of " + str(
                                            self._goal_vic) + " as you did not come.", 'RescueBot')
                                    elif 'mild' in self._goal_vic:
                                        ...
                                        self._waiting = False
                                        self._send_message(
                                            "Rescuing " + str(self._goal_vic) + " alone as you did not come.",
                                            'RescueBot')
            ...
            for msg in mssgs:
                ...
                if msg.startswith("Search:"):
                    ...
                    if ((self._trustBeliefs[self._human_name]['willingness'] > 0 or random.random() > 0.1)
                        ...
                        self._searched_by_human.append(area)
    ...
    def _loadBelief(self, members, folder):
        ...
        with open('trust_logs.csv', 'w', newline="") as file:
    ...
    def _trustBelief(self, members, folder, receivedMessages, state):
        ...
        if self._last_processed_trust + 1 < len(receivedMessages):
            message = receivedMessages[-1]
            self._last_processed_trust += 1
            print("Processing new message: ")
            # If the human told the robot to continue, we decrease their willingness
            if 'Continue' in message:
                self.alter_belief('willingness', -0.1, state)
                print("Detected a Continue - willingness -0.1")
            # If the human told the robot search in an already searched room, the willingness gets decreased
            ...
            if 'Collect' in message:
                ...
                if victim in self._found_victims:
                    # If the human picked up a victim that was marked as found, we increase the willigneness and competence
                    self.alter_belief('competence', 0.3, state)
                    self.alter_belief('willingness', 0.1, state)
            ...
            if 'Remove:' in message:
                ...
                # we decrease the willingness for lying
